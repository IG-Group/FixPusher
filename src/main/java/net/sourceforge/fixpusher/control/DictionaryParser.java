...
public class DictionaryParser {
	...
	public DictionaryParser(final String file, final String file2) {
		...
		try {
			...
			dataDictionaryDocument = builder.parse(new InputSource(getClass().getClassLoader().getResourceAsStream("conf/userfields.xml")));
		...
		try {
			...
			final DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
			final DocumentBuilder builder = factory.newDocumentBuilder();
			dataDictionaryDocument = builder.parse(new InputSource( getClass().getClassLoader().getResourceAsStream(file)));
		}
		catch (final Exception e) {
